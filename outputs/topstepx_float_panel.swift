...
final class PanelController: NSObject, NSApplicationDelegate, NSTextFieldDelegate {
    ...
    var lastKnownPositionSide: String = "FLAT"
    var lastNonFlatPositionSide: String = "FLAT"
    var lastNonFlatPositionAt: Date = .distantPast
    var submittedEntryOrderIds: Set<Int> = []
    var protectionOrderType: [Int: Int] = [:]  // id -> 1 TP, 4 SL etc for sound decision on fills
    var protectionOrderKind: [Int: String] = [:]  // id -> TP or SL from local submit intent
    var lastProtectionFillSound: (id: Int, time: Date)?
    var recentFillSoundKeys: [String: Date] = [:]
    private var lastAnyFillSoundAt: Date = .distantPast
    var knownSnapshotTradeSoundKeys: Set<String>?
    var snapshotTradeSoundAccountId: Int?
    var suppressSnapshotFillSoundsUntil: Date?
...
extension PanelController {
    ...
    @discardableResult
    func playFillSoundFile(_ kind: String) -> Bool {
        let now = Date()
