...
SELECTION_SRC="$SCRIPT_DIR/PanelController+Selection.swift"
REALTIME_STREAMS_SRC="$SCRIPT_DIR/PanelController+RealtimeStreams.swift"
REALTIME_PAYLOADS_SRC="$SCRIPT_DIR/PanelController+RealtimePayloads.swift"
POSITION_ACCOUNT_SRC="$SCRIPT_DIR/PanelController+PositionAccount.swift"
...
echo "Selection: $SELECTION_SRC"
echo "Realtime Streams: $REALTIME_STREAMS_SRC"
echo "Realtime Payloads: $REALTIME_PAYLOADS_SRC"
echo "Position Account: $POSITION_ACCOUNT_SRC"
echo "Main: $MAIN_SRC"
...
xcrun swiftc -sdk "$(xcrun --show-sdk-path)" \
  ...
  "$CORE_SRC" "$CONTROLS_SRC" "$VIEWS_SRC" "$SRC" "$LAYOUT_SRC" "$TEXT_SRC" "$TICKET_UI_SRC" "$TICKET_STATE_SRC" "$WORKING_ORDERS_SRC" "$REFRESH_SRC" "$SELECTION_SRC" "$REALTIME_STREAMS_SRC" "$REALTIME_PAYLOADS_SRC" "$POSITION_ACCOUNT_SRC" "$MAIN_SRC" -o "$ARM64_BIN"
...
xcrun swiftc -sdk "$(xcrun --show-sdk-path)" \
  ...
  -framework AppKit -framework Foundation \
  -file-prefix-map "$SCRIPT_DIR=." \
  "$CORE_SRC" "$CONTROLS_SRC" "$VIEWS_SRC" "$SRC" "$LAYOUT_SRC" "$TEXT_SRC" "$TICKET_UI_SRC" "$TICKET_STATE_SRC" "$WORKING_ORDERS_SRC" "$REFRESH_SRC" "$SELECTION_SRC" "$REALTIME_STREAMS_SRC" "$REALTIME_PAYLOADS_SRC" "$POSITION_ACCOUNT_SRC" "$MAIN_SRC" -o "$X86_BIN"
